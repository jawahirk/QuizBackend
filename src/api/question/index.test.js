...
test('POST /questions 201 (admin)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: adminSession, question: 'test', type: 'test', options: 'test' })
...
test('GET /questions 200 (master)', async () => {
  const { status, body } = await request(app())
    .get(`${apiRoot}`)
...
test('GET /questions 401 (admin)', async () => {
  const { status } = await request(app())
    .get(`${apiRoot}`)
...
test('GET /questions/:id 200 (master)', async () => {
  const { status, body } = await request(app())
...
test('GET /questions/:id 401 (admin)', async () => {
  const { status } = await request(app())
    .get(`${apiRoot}/${question.id}`)
    .query({ access_token: adminSession })
  expect(status).toBe(401)
...
test('GET /questions/:id 404 (master)', async () => {
  const { status } = await request(app())
